...
class Player(pygame.sprite.Sprite):
    ...
    def import_dust_run_particles(self):
        self.dust_run_particles = import_folder('../resources/graphics/'
                                                'character/dust_particles/run')
    ...
    def dust_run_animation(self):
        if self.status == 'run' and self.on_ground:
            ...
            dust = self.dust_run_particles[int(self.dust_frame_index)]
            if self.facing_right:
                pos = self.rect.bottomleft - pygame.math.Vector2(6, 10)
                self.display_surface.blit(dust, pos)
            else:
                pos = self.rect.bottomright - pygame.math.Vector2(6, 10)
                dust = pygame.transform.flip(dust, True, False)
                self.display_surface.blit(dust, pos)
